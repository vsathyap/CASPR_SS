function [d,d_interm, cf, sd_oct_mat] = stoi(x, y, fs_signal)
...
sd_oct_mat     = zeros(J, size(x_hat, 2));    % init memory for clean energy 1/3 octave band TF-representation 

for i = 1:size(x_hat, 2)
    X(:, i)	= sqrt(H*abs(x_hat(:, i)).^2);      % apply 1/3 octave bands as described in Eq.(1)
    Y(:, i)	= sqrt(H*abs(y_hat(:, i)).^2);

    sd_oct_mat(:, i) = abs(10*log10(X(:,i).^2) - 10*log10(Y(:,i).^2));  % sd in  1/3 octave bands 
end
